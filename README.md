...
The main external dependancies are the blazingly the alignment libraries
...
output into or read input from other streams. There is a bash script in examples
showing how to do this in a parallelized fashion.

The full flow of analysis is as follows (although some of the following steps
can be skipped):

### mer

Bayesian merge paired end reads and trim them (skipped if a single file is supplied).
...
paired reads with adapters of the form
...
(can also direct this to stdout) and another output csv file which describes
for each sequence some properties of the merge and adapter alignment.
The output format is described in [Outputs](#outputs) section.
...
Align references to reads and identify MIDs.

Given references, they and their reverse complements are aligned 
to the merged reads to identify which references align a given read the best. 
...
ign produces one output file csv which describes alignment propoerties of MID
and reference alignments for each sequence. The output format is described in 
...
This package was put together for the paper [ref4](link) therefore its main target
are libraries of relatively short reads (~300bp) where samples comprimise of
variants of known reference strains and the aim is to map each read to one
of these variants along side with any substitutions it has acquired. In particular
in this paper it was applied to samples majorly consisting of single substitution 
variants of BE92 influenza virus in seven different positions. 
...
The other output which is either directed to a file of supplied or to stdout 
is a list of merged sequences and their merged quality scores. See introduction for details.
...
to [mid_start, mid_start + MID length + mid_buffer] and 
...
Alongside the unit tests done via [Check](https://libcheck.github.io/check/), 
coverage tests are performed via [gcov](https://gcc.gnu.org/onlinedocs/gcc/Gcov.html)
and memmory tests are performed via [Valgrind](https://valgrind.org/).
...
<br />
Running suite(s): STRUCTURES
100%: Checks: 5, Failures: 0, Errors: 0
<br />
Running suite(s): BAD INPUTS
100%: Checks: 7, Failures: 0, Errors: 0
<br />
Running suite(s): ALIGNMENT
100%: Checks: 10, Failures: 0, Errors: 0
<br />
Running suite(s): COVERAGE
